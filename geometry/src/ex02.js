...
export default function example() {
	...
	for(let i = 0; i < positionArray.length; i+= 3) {
		...
		positionArray[i] = positionArray[i] + (Math.random() - 0.5) * .2;
		positionArray[i + 1] = positionArray[i + 1] + (Math.random() - 0.5) * .2;
		positionArray[i + 2] = positionArray[i + 2] + (Math.random() - 0.5) * .2;
	...
	function draw() {
		...
		for(let i = 0; i < positionArray.length; i += 3) {
			// positionArray[i] = positionArray[i] + Math.sin() 아래처럼 함축 가능
